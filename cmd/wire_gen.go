...
func initApplication(debug bool, serverConf *conf.Server, dbConf *data.Database, cacheConf *data.CacheConf, i18nConf *translator.I18n, swaggerConf *router.SwaggerConfig, serviceConf *service_config.ServiceConfig, uiConf *server.UI, logConf log.Logger) (*pacman.Application, func(), error) {
	...
	repository := personal_access_token.NewRepository(dataData)
	personal_access_tokenService := personal_access_token2.NewService(repository)
	userService := content.NewUserService(userRepo, userActiveActivityRepo, activityRepo, emailService, authService, siteInfoCommonService, userRoleRelService, userCommon, userExternalLoginService, userNotificationConfigRepo, userNotificationConfigService, questionCommon, eventqueueService, fileRecordService, personal_access_tokenService)
	...
	personalAccessTokenController := controller.NewPersonalAccessTokenController(personal_access_tokenService, siteInfoCommonService, userCommon)
	...
	patRequestAuthorizer := middleware.NewPATRequestAuthorizer(personal_access_tokenService, authService, siteInfoCommonService)
