...
func initApplication(debug bool, serverConf *conf.Server, dbConf *data.Database, cacheConf *data.CacheConf, i18nConf *translator.I18n, swaggerConf *router.SwaggerConfig, serviceConf *service_config.ServiceConfig, uiConf *server.UI, logConf log.Logger) (*pacman.Application, func(), error) {
	...
	service := eventqueue.NewService()
	activityLogService := activity_log2.NewActivityLogService(activityLogRepo, commentCommonRepo, service)
	userRepo := user.NewUserRepo(dataData, activityLogService)
	...
	userRankRepo := rank.NewUserRankRepo(dataData, configService, activityLogService)
	...
	activityqueueService := activityqueue.NewService()
	...
	reviewService := review2.NewReviewService(reviewRepo, objService, userCommon, userRepo, questionRepo, answerRepo, userRoleRelService, externalService, tagCommonService, questionCommon, noticequeueService, siteInfoCommonService, commentCommonRepo, vector_syncService, activityLogService)
	commentService := comment2.NewCommentService(commentRepo, commentCommonRepo, userCommon, objService, voteRepo, emailService, userRepo, noticequeueService, externalService, activityqueueService, service, reviewService, vector_syncService)
	...
	voteService := content.NewVoteService(contentVoteRepo, configService, questionRepo, answerRepo, commentCommonRepo, objService, service, activityLogService)
	...
	contentRevisionService := content.NewRevisionService(revisionRepo, userCommon, questionCommon, answerService, objService, questionRepo, answerRepo, tagRepo, tagCommonService, noticequeueService, activityqueueService, reportRepo, reviewService, reviewActivityRepo)
	...
	templateController := controller.NewTemplateController(templateRenderController, siteInfoCommonService, service, userService, questionService)
	...
	scheduledTaskManager := cron.NewScheduledTaskManager(siteInfoCommonService, questionService, fileRecordService, userAdminService, serviceConf, activityLogService)
